...
glm::vec3	CMyApp::GetPos(float u, float v)
{
	...
	return glm::vec3(-(R + r * cv) * cu, r * sv, (R + r * cv) * su);
...
void CMyApp::Update()
{
	...
	float t = SDL_GetTicks()/1000.0f, d = 10;
	m_matView = glm::lookAt(glm::vec3( d*cosf(t),  d,  d*sinf(t)),		// honnan nézzük a színteret
							glm::vec3( 0,  0,  0),		// a színtér melyik pontját nézzük
							glm::vec3( 0,  1,  0));		// felfelé mutató irány a világban
}


void CMyApp::drawTorus(int i)
{
	glUseProgram(m_programID);
	//m_matWorld = glm::mat4(1.0f);

	//glm::mat4 mvp = m_matProj * m_matView * m_matWorld;
	m_matWorld = glm::rotate<float>(SDL_GetTicks() / 1000.0 * M_PI / (float)i, glm::vec3(0, 1, 0)) * glm::translate<float>(glm::vec3(8, 0, 0));
	glm::mat4 mvp = m_matProj * m_matView * m_matWorld;
