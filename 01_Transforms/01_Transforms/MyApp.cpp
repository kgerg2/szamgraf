...
void CMyApp::Update()
{
	float t = SDL_GetTicks() / 2500.0 * M_PI;
...
void CMyApp::Render()
{
	...
	GLM transzformációs mátrixokra példák:
		glm::rotate<float>( szög (rad), glm::vec3(tengely_x, tengely_y, tengely_z) ) <- tengely_{xyz} körüli elforgatás
		glm::translate<float>( glm::vec3(eltol_x, eltol_y, eltol_z) ) <- eltolás
		glm::scale<float>( glm::vec3(s_x, s_y, s_z) ) <- léptékezés

	*/
	m_matWorld = glm::mat4(1.0f); // egységmátrix
