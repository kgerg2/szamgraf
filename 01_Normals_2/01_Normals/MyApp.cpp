...
glm::vec3 CMyApp::GetNorm(float u, float v)
{
	...
	return glm::vec3(su * sv, cv, cu * sv); // ugyanaz, csak nem szorozva sugárral
...
void CMyApp::InitSphere()
{
	...
	for (int i = 0; i < N; i++)
	{
		vert[(N + 1) * (M + 1) + i].p = glm::vec3();
		vert[(N + 1) * (M + 1) + i].n = glm::vec3();
		vert[(N + 1) * (M + 1) + i].t = glm::vec2();
	}
