...
void CMyApp::InitSphere()
{
	...
	for (int i = 0; i < N; ++i)
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0); // feltöltöttük a IB-t is, ezt is vegyük le
#else

	float height = 2;

	std::vector<Vertex> vert;
	vert.push_back({ glm::vec3(0, 0, 0), glm::vec3(0,-1, 0), glm::vec2(0,0) }); //bottom midde point
	vert.push_back({ glm::vec3(0, height, 0), glm::vec3(0, 1, 0), glm::vec2(1,1) }); //top midde point

	for (size_t i = 0; i < N; ++i) {
		float alpha = i / (float)N * 2 * M_PI;
		vert.push_back({ glm::vec3(cos(alpha), 0, sin(alpha)), glm::vec3(cos(alpha), 0, sin(alpha)), glm::vec2(i / (float)N, 0) });
	}

	for (size_t i = 0; i < N; ++i) {
		float alpha = i / (float)N * 2 * M_PI;
		vert.push_back({ glm::vec3(cos(alpha), height, sin(alpha)), glm::vec3(cos(alpha), 0, sin(alpha)), glm::vec2(i / (float)N, 1) });
	}
	...
	// töltsük fel adatokkal az aktív VBO-t
	glBufferData(GL_ARRAY_BUFFER,	// az aktív VBO-ba töltsünk adatokat
		sizeof(Vertex)*vert.size(),		// ennyi bájt nagyságban
		vert.data(),	// errõl a rendszermemóriabeli címrõl olvasva
		GL_STATIC_DRAW);	// úgy, hogy a VBO-nkba nem tervezünk ezután írni és minden kirajzoláskor felhasnzáljuk a benne lévõ adatokat
...
void CMyApp::Render()
{
	...
	// feladat - textúra
	glm::vec3 location = m_camera.GetEye();
	glUniform3fv(glGetUniformLocation(m_programID, "eye_pos"), 1, &location.x);
	...
	glDrawElements(GL_TRIANGLES,	// primitív típus
		12*N,			// hany csucspontot hasznalunk a kirajzolashoz
